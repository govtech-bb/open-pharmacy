/* =====================================================================
   open-pharmacy prototype — select-to-comment review widget
   ---------------------------------------------------------------------
   ...
   STORAGE — read this:
   This build stores comments in a Supabase project (Postgres + REST), so
   comments ARE shared between everyone. The anon public key below is safe
   to ship in the page. Run the SQL in supabase-setup.sql once to create
   the table + access policies.

   If the anon key is left as a placeholder, the widget falls back to
   localStorage (per-browser, NOT shared) so it still works meanwhile.
   ===================================================================== */
(function () {
  "use strict";
  ...
  // Supabase REST store. Talks directly to PostgREST with the anon key.
  // Requires the `comments` table + RLS policies from supabase-setup.sql.
  ...
  var hasSupabase = COMMENTS_CONFIG.supabase &&
    COMMENTS_CONFIG.supabase.anonKey &&
    COMMENTS_CONFIG.supabase.anonKey.indexOf("PASTE_") !== 0;
  var store = hasSupabase
    ? SupabaseStore(COMMENTS_CONFIG.supabase, COMMENTS_CONFIG.pageId)
    : LocalStore(COMMENTS_CONFIG.pageId);
  if (!hasSupabase && window.console) {
    console.warn("[comments] No Supabase anon key set — using per-browser localStorage. Comments will NOT be shared. Set COMMENTS_CONFIG.supabase.anonKey in comments.js.");
  }
